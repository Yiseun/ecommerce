...
public class ConstraintTests {
    ...
    void 매개변수가_비어있다면_실패한다(){
        final Constraint sut = Constraint.from(null);
    ...
    void 자신의_tryCount가_null이라면_실패한다(){
        final Constraint sut = Constraint.createEmpty();
    ...
    void 자신의_tryCount가_다_떨어졌다면_실패한다(){
        final Constraint sut = Constraint.from(-1);
        final Constraint request = Constraint.createEmpty();

        assertThatThrownBy(()->sut.update(request)).isInstanceOf(BusinessLogicException.class);
    }
}
